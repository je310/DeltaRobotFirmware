...
int Kinematics::goToWorldPos(Eigen::Affine3f currentPos, Eigen::Affine3f targetPos,Eigen::Vector3f angRates, Eigen::Affine3f outPos, Eigen::Affine3f &outPos2, float ffGain, Eigen::Vector3f &deltaPos,float &yawOut,float &pitchOut){
    ...
    Eigen::Affine3f targetNoRot  = targetPos * Eigen::AngleAxisf(-3.14/2.0, Eigen::Vector3f::UnitX());
    ...
    if(dif < range){
        ...
        goToPos(kinTrans[0]*1000, kinTrans[1]*1000, kinTrans[2]*1000);
        kinOut.translation() = kinTrans;
        outPos = origin * kinOut * HeadCentreToPitch * pitch * PitchToYaw * yaw * yawToTip * roll;
